...
export default function PageProveedores() {
  ...
  */
  const getData = async () => {
    setLoading(false);
    const dataFetch = await getDataFetch('file', 'supplier')
  ...
  const getDataFilter = (fil) => {
    ...
    if (myData && myData !== undefined) {
      ...
      switch (fil) {
        ...
        case 'supplies_services': setLoadSupplier(filteredData); break;
  ...
  useEffect(() => {
    ...
    getDataFilter('supplies_services')
    ...
    <Layout>
      <div className={`${styles.page_container}`} >
        ...
        <div className={`${styles.page_content}`}>
          <SectionSupplier data={loadSupplier} />
          <SectionForms data={loadForm} type={'supplier'} />
        </div>
      </div>
    </Layout>
  );
